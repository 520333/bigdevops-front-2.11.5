<template>
  <div>
      ...
      <template #toolbar>
        <div class="flex items-center space-x-3 flex-wrap gap-y-2">
          ...
          <a-input-search
            ...
            @search="handleReload"
          />
          <a-button type="primary" @click="handleCreateIng">
            新增 Ingress
          </a-button>

          <a-popconfirm
            title="确定要批量删除选中的 Ingress 吗？"
            ok-text="确认删除"
            cancel-text="取消"
            ...
            <TableAction
              :actions="[
                ...
                },
                {
                  ...
                  tooltip: '删除 Ingress',
                  popConfirm: {
                    title: `确认删除 Ingress [${record.name}] 吗？`,
                    confirm: handleDeleteIng.bind(null, record),
                  },
                },
              ]"
            />
    ...
    <a-modal
      ...
      :title="isCreate ? '新增 Ingress (YAML 声明)' : `编辑 Ingress: ${activeName}`"
      width="65%"
    ...
    >
      <div class="flex items-center justify-between mb-3">
        <div class="text-xs text-gray-500">
          {{ isCreate ? '请编写完整 Ingress 7层路由规则 YAML 声明。' : '修改 YAML 内容后点击确定，系统将在线更新 Ingress 路由配置。' }}
        </div>
...
<script lang="ts" setup>
  ...
  const ingYamlContent = ref('');

  const defaultIngYaml = `apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
...
`;
  ...
  const columns: BasicColumn[] = [
    { title: 'Ingress 名称', dataIndex: 'name', width: 220, ellipsis: true },
    {
      title: '命名空间',
      dataIndex: 'namespace',
      width: 130,
    ...
    { title: 'Ingress Class', dataIndex: 'class', width: 140, customRender: ({ record }) => record.class || '-' },
    {
      title: '域名 Hosts',
      dataIndex: 'hosts',
      width: 220,
      customRender: ({ record }) => (record.hosts || []).join(', ') || '*',
    },
    { title: 'LB IP / 负载地址', dataIndex: 'address', width: 160, customRender: ({ record }) => record.address || '-' },
    { title: '开放端口', dataIndex: 'ports', width: 110, customRender: ({ record }) => record.ports || '80' },
  ...
  const [registerTable, { reload, getDataSource }] = useTable({
    title: 'Ingress 7层应用路由列表',
    api: async () => {
      if (!selectedCluster.value) return { items: [], total: 0 };
      try {
        const res = await getK8sIngressList({
        ...
        console.error(e);
        createMessage.error('获取 Ingress 列表失败: ' + (e.message || e));
  ...
  function downloadCurrentYaml() {
    const filename = activeName.value ? `${activeName.value}.yaml` : 'ingress.yaml';
    triggerDownload(filename, ingYamlContent.value);
    createMessage.success(`YAML 文件 [${filename}] 下载中...`);
  }

  async function handleDownloadRowYaml(record: Recordable) {
    try {
      createMessage.loading({ content: `正在读取 Ingress [${record.name}] YAML...`, key: 'download_ing_yaml' });
      const yamlStr = await getK8sIngressYaml({ clusterName: selectedCluster.value, namespace: record.namespace, name: record.name });
      createMessage.destroy('download_ing_yaml');
      const content = typeof yamlStr === 'string' ? yamlStr : JSON.stringify(yamlStr, null, 2);
      triggerDownload(`${record.name}.yaml`, content);
      createMessage.success(`Ingress [${record.name}.yaml] 下载成功`);
    } catch (e: any) {
      console.error(e);
      createMessage.error({ content: '获取 YAML 失败: ' + (e.message || e), key: 'download_ing_yaml' });
    }
  }
  ...
  function handleCreateIng() {
    isCreate.value = true;
    activeName.value = '';
    ingYamlContent.value = defaultIngYaml;
    modalVisible.value = true;
  }

  async function handleEditYaml(record: Recordable) {
    try {
      createMessage.loading({ content: `正在读取 Ingress [${record.name}] YAML...`, key: 'load_ing_yaml' });
      const yamlStr = await getK8sIngressYaml({ clusterName: selectedCluster.value, namespace: record.namespace, name: record.name });
      createMessage.destroy('load_ing_yaml');
      isCreate.value = false;
      activeName.value = record.name;
      activeNamespace.value = record.namespace;
      ingYamlContent.value = typeof yamlStr === 'string' ? yamlStr : JSON.stringify(yamlStr, null, 2);
      modalVisible.value = true;
    } catch (e: any) {
      console.error(e);
      createMessage.error({ content: '获取 YAML 失败: ' + (e.message || e), key: 'load_ing_yaml' });
    }
  }

  async function handleSaveIng() {
    if (!ingYamlContent.value.trim()) return createMessage.warning('YAML 内容不能为空');
    modalLoading.value = true;
    try {
      if (isCreate.value) {
        await createK8sIngress({ clusterName: selectedCluster.value, namespace: selectedNamespace.value, yamlContent: ingYamlContent.value });
        createMessage.success('Ingress 创建成功');
      } else {
        await updateK8sIngress({ clusterName: selectedCluster.value, namespace: activeNamespace.value, yamlContent: ingYamlContent.value });
        createMessage.success('Ingress 更新成功');
      }
  ...
  async function handleDeleteIng(record: Recordable) {
    try {
      createMessage.loading({ content: `正在删除 Ingress [${record.name}]...`, key: 'del_ing' });
      await deleteK8sIngress({ clusterName: selectedCluster.value, namespace: record.namespace, name: record.name });
      createMessage.success({ content: `Ingress [${record.name}] 删除成功`, key: 'del_ing' });
      await reload();
    } catch (e: any) {
      console.error(e);
      createMessage.error({ content: '删除失败: ' + (e.message || e), key: 'del_ing' });
    }
  }
  ...
  async function handleBatchDelete() {
    ...
    try {
      ...
      createMessage.loading({ content: `正在批量删除 ${items.length} 个 Ingress...`, key: 'batch_del_ing' });
      await deleteK8sIngressBatch({ clusterName: selectedCluster.value, items });
      createMessage.success({ content: '批量删除操作完成', key: 'batch_del_ing' });
      checkedKeys.value = [];
      await reload();
    } catch (e: any) {
      console.error(e);
      createMessage.error({ content: '批量删除失败: ' + (e.message || e), key: 'batch_del_ing' });
    }
  }
</script>
