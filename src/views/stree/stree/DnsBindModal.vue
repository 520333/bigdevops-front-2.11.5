<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="关联 DNS 资源到当前节点" :width="960" :destroyOnClose="true"
...
import { Table, Tag, Input, Button, Radio } from 'ant-design-vue';
import { getResourceDnsUnbindList, bindDnsToStreeNode } from '@/api/demo/system';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const pagination = reactive({
      ...
      showTotal: (total: number) => `共 ${total} 条待关联域名`,
    ...
    const [registerModal, { closeModal }] = useModalInner(async (data) => {
      ...
      pagination.current = 1;
      await fetchAvailableDns();
    });

    const fetchAvailableDns = async () => {
      if (!targetNode.value.id) return;
      loading.value = true;
      try {
        const res: any = await getResourceDnsUnbindList();
        rawList.value = Array.isArray(res) ? res : (res?.data || []);
      } catch (err: any) {
        console.error('获取待关联 DNS 失败', err);
      } finally {
        loading.value = false;
      }
    };
    ...
    const handleSubmit = async () => {
      if (selectedRowKeys.value.length === 0) {
        createMessage.warning('请至少选择一条域名解析进行关联');
        return;
      }

      submitting.value = true;
      try {
        await bindDnsToStreeNode({
          node_id: targetNode.value.id,
          resource_ids: selectedRowKeys.value,
        });
        createMessage.success(`成功关联 ${selectedRowKeys.value.length} 条域名到【${targetNode.value.title}】！`);
        closeModal();
        emit('success', { nodeId: targetNode.value.id });
      } catch (err: any) {
        console.error('绑定 DNS 失败', err);
      } finally {
