<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="批量解绑当前节点的 ECS 资源" :width="960" :destroyOnClose="true"
...
import { unBindEcsToStreeNode, fetchResourceByNode } from '@/api/demo/system';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const pagination = reactive({
      ...
      showTotal: (total: number) => `共 ${total} 台已绑定主机`,
    ...
    const fetchBoundEcs = async () => {
      if (!targetNode.value.id) return;
      loading.value = true;
      try {
        if (targetNode.value.bind_ecss && targetNode.value.bind_ecss.length > 0) {
          rawList.value = targetNode.value.bind_ecss;
        }
        // 同步从接口拉取最新已绑定主机数据
    ...
    const handleSubmit = async () => {
      if (selectedRowKeys.value.length === 0) {
        createMessage.warning('请至少选择一台主机进行解绑');
        return;
      }

      submitting.value = true;
      try {
        await unBindEcsToStreeNode({
          node_id: targetNode.value.id,
          resource_ids: selectedRowKeys.value,
        });
        createMessage.success(`成功从【${targetNode.value.title}】解绑 ${selectedRowKeys.value.length} 台主机！`);
        closeModal();
